...
// forked from: https://github.com/shadcn-ui/ui/blob/main/apps/www/scripts/build-registry.mts
...
const REGISTRY_PATH = path.join(process.cwd(), "build-registry");
...
`;
  ...
  rmSync(path.join(REGISTRY_PATH, "index.json"), {
    recursive: true,
    force: true,
  });
  await fs.writeFile(
  ...
  if (!existsSync(path.join(process.cwd(), "__registry__"))) {
    mkdirSync(path.join(process.cwd(), "__registry__"), { recursive: true });
  }

  // Write style index.
  rmSync(path.join(process.cwd(), "__registry__/index.tsx"), {
    recursive: true,
    force: true,
  });
...
async function buildThemes() {
  ...
  rmSync(colorsTargetPath, { recursive: true, force: true });
...
.dark .theme-<%- theme %> {
  ...
  --ring: <%- colors.dark["ring"] %>;
    ...
    // ----------------------------------------------------------------------------
    rmSync(path.join(REGISTRY_PATH, "themes"), {
      recursive: true,
      force: true,
    });
    for (const baseColor of ["slate", "gray", "zinc", "neutral", "stone"]) {
