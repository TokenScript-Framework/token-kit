...
```bash
npx @token-kit/cli-ui@latest init
```
...
```bash
npx @token-kit/cli-ui@latest add
```
...
```bash
npx @token-kit/cli-ui diff alert
```
...
We are shipping two styles: `default` and `default` (with more coming soon).
...
The `default` style is a new style. It ships with smaller buttons, cards with shadows and a new set of icons from [Radix Icons](https://icons.radix-ui.com).
